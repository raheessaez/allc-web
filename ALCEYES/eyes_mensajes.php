...
<td >
		...
		if (empty($B_FECHA_E)) { $B_FECHA_E=@$_GET["B_FECHA_E"]; }
				...
				//CALCULAR MINIMO Y MÁXIMO FECHA EVENT_DATE
				$CONSULTA2="SELECT MAX(EVENT_DATE) AS MINFECHA FROM FE_MESSAGE";
				...
				$CONSULTA2="SELECT MAX(EVENT_DATE) AS MAXFECHA FROM FE_MESSAGE";
				$RS2 = sqlsrv_query($conn, $CONSULTA2);
				//oci_execute($RS2);
				if ($row = sqlsrv_fetch_array($RS2)){
		...
		$F_FECHA=" WHERE Convert(varchar(20), EVENT_DATE, 111) >=  Convert(varchar(20),'".$FECHA_ED."', 111) AND Convert(varchar(20), EVENT_DATE, 111) <= Convert(varchar(20),'".$FECHA_EH."', 111)"; 
		?>
		<script>
         function EyesMensajes(){
                var contenedor = document.getElementById("MessageContainer");
                contenedor.style.display = "block";
                document.getElementById('MessageContainer').innerHTML = "<object type='text/html' data='eyes_msjeActivo.php?dia_ed=<?php echo $dia_ed?>&mes_ed=<?php echo $mes_ed?>&ano_ed=<?php echo $ano_ed?>&dia_eh=<?php echo $dia_eh?>&mes_eh=<?php echo $mes_eh?>&ano_eh=<?php echo $ano_eh?>' id='MonitorIframe' onload='setIframeHeight(this.id)'></object>";
                return true;
            }
         ...
         function EyesStoreMensajes(val){
                ...
                contenedor.style.display = "block";
                document.getElementById('MessageStoreContainer').innerHTML = "<object type='text/html' data='eyes_msjeTienda.php?idl="+IdLocal+"&dia_ed=<?php echo $dia_ed?>&mes_ed=<?php echo $mes_ed?>&ano_ed=<?php echo $ano_ed?>&dia_eh=<?php echo $dia_eh?>&mes_eh=<?php echo $mes_eh?>&ano_eh=<?php echo $ano_eh?>' id='MonitorIframe' onload='setIframeHeight(this.id)'></object>";
                return true;
            }
         ...
         function EyesHardwareMensajes(val, ideq){
                ...
                var contenedor = document.getElementById("MessageStoreContainer");
                contenedor.style.display = "block";
                document.getElementById('MessageStoreContainer').innerHTML = "<object type='text/html' data='eyes_msjeTienda.php?idl="+IdLocal+"&ideq="+IdEquipo+"&dia_ed=<?php echo $dia_ed?>&mes_ed=<?php echo $mes_ed?>&ano_ed=<?php echo $ano_ed?>&dia_eh=<?php echo $dia_eh?>&mes_eh=<?php echo $mes_eh?>&ano_eh=<?php echo $ano_eh?>' id='MonitorIframe' onload='setIframeHeight(this.id)'></object>";
                return true;
            }
        </script>
        <table width="100%" id="Filtro">
          <tr>
            <td>
                <form action="eyes_mensajes.php?B_FECHA_E=<?php echo $B_FECHA_E ?>&dia_ed=<?php echo $dia_ed ?>&mes_ed=<?php echo $mes_ed ?>&ano_ed=<?php echo $ano_ed ?>&dia_eh=<?php echo $dia_eh ?>&mes_eh=<?php echo $mes_eh ?>&ano_eh=<?php echo $ano_eh ?>" method="post" name="frmbuscar" id="frmbuscar">
                    ...
                    <input name="LIMPIAR" type="button" id="LIMPIAR" value="Limpiar" onClick="javascript:pagina('eyes_mensajes.php')">
        ...
        <tr>
            <td width="20px">
                        ...
                        <?php
                                $SQL="SELECT ID_LOCAL FROM FE_MESSAGE ".$F_FECHA." GROUP BY ID_LOCAL ORDER BY ID_LOCAL";
                                ...
                                while ($row = sqlsrv_fetch_array($RS)){
                                        ...
                                        <style>
                                                ...
                                                <div id="StoreSel<?=$ID_LOCAL;?>" style="display:none" onClick="EyesStoreMensajes(<?php echo $ID_LOCAL;?>);">
                                                        ...
                                                        <div id="CloseSel" onClick="pagina('eyes_mensajes.php?dia_ed=<?php echo $dia_ed?>&mes_ed=<?php echo $mes_ed?>&ano_ed=<?php echo $ano_ed?>&dia_eh=<?php echo $dia_eh?>&mes_eh=<?php echo $mes_eh?>&ano_eh=<?php echo $ano_eh?>');">
                                                        </div>
                                                </div>
                                            ...
                                            <div id="HardwareList<?=$ID_LOCAL?>" style="display:none">
											<?php
                                                    $SQL1="SELECT * FROM FM_EQUIPO  WHERE ID_LOCAL=".$ID_LOCAL." AND ID_EQUIPO IN(SELECT ID_EQUIPO FROM FE_MESSAGE ".$F_FECHA.") ORDER BY ID_TIPO ASC, DES_CLAVE ASC";
                                                    ...
                                                    while ($row1 = sqlsrv_fetch_array($RS1)){
																	...
																	$SQL2="SELECT * FROM FM_TIPO_EQUIPO WHERE ID_TIPO=".$ID_TIPO;
																	$RS2 = sqlsrv_query($conn, $SQL2);
																	//oci_execute($RS2);
																	if ($row2 = sqlsrv_fetch_array($RS2)){
																		$DES_TIPO = $row2['DES_TIPO'];
																	}
                                ...
                                while ($row = sqlsrv_fetch_array($RS)){
                                        ...
                                        <script>
										 function SelTienda<?php echo $ID_LOCAL;?>(){
												...
												<?php
												$SQL1="SELECT ID_LOCAL FROM FE_MESSAGE  ".$F_FECHA." AND ID_LOCAL<>".$ID_LOCAL." GROUP BY ID_LOCAL ORDER BY ID_LOCAL";
												$RS1 = sqlsrv_query($conn, $SQL1);
												//oci_execute($RS1);
												while ($row1 = sqlsrv_fetch_array($RS1)){
