...
<?php
if ($LIST==1) {
				$S="SELECT COUNT(IDSISTEMA) AS CUENTA FROM US_SISTEMA";
...
<?php if($LIST==1) { ?>                
                ...
                $S= "SELECT * FROM (SELECT *,ROW_NUMBER() OVER (PARTITION BY 2 ORDER BY NOMBRE ASC) ROWNUMBER FROM US_SISTEMA WHERE IDSISTEMA <> 0) AS TABLEWITHROWNUMBER WHERE ROWNUMBER BETWEEN ".$LINF." AND ".$LSUP."";
               ...
               ?>
                <tr>
                    <?php if($SESPUBLICA==1) { ?>
                    <td><a href="mant_sistemas.php?ACT=<?php echo $IDSISTEMA?>"><span style="font-weight:600;"><?=$NOMBRE." (". $IDSISTEMA.")"?></span></a></td>
                    <?php } else {?>
