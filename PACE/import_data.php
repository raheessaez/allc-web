...
//$replace => caracter para reemplazar, debe tener la misma cantidad y estructura que $char, por default el valor es ""
...
if(json_encode($obj)!=false)
{
	...
	if(fopen('IN/FILE0.JSON', 'w+')!=false)
	{
		...
		if(fwrite($fp, $json_file)!=false)
		{
			...
			fclose($file_);
			echo 'Proceso Exitoso <a href="IN/FILE0.JSON" target="_blank">FILE0.JSON</a> con '.filesize("IN/FILE0.JSON").'<br>';
			echo 'Proceso Exitoso <a href="IN/EAMOPTNS_PASO.INI" target="_blank">EAMOPTNS_PASO.INI</a> con '.filesize("IN/EAMOPTNS_PASO.INI").'<br>';
		}
		else
		{
...
{
	$json_file=json_encode($obj);
	if(fopen('IN/FILE_CAMPOS.JSON', 'w+')!=false)
	{
		$fp = fopen('IN/FILE_CAMPOS.JSON', 'w+');
		if(fwrite($fp, $json_file)!=false)
		{
			fclose($fp);
			echo 'Proceso Exitoso <a href="IN/FILE_CAMPOS.JSON" target="_blank">FILE_CAMPOS.JSON</a> con '.filesize("IN/FILE_CAMPOS.JSON").'<br>';
		}
		else
		{
...
{
	$json_file=json_encode($obj);
	if(fopen('IN/FILE_LIST.JSON', 'w+')!=false)
	{
		$fp = fopen('IN/FILE_LIST.JSON', 'w+');
		if(fwrite($fp, $json_file)!=false)
		{
			fclose($fp);
			echo 'Proceso Exitoso <a href="IN/FILE_LIST.JSON" target="_blank">FILE_LIST.JSON</a> con '.filesize("IN/FILE_LIST.JSON").'<br>';
		}
		else
		{
