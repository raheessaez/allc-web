...
//	$FPAGO="";
	...
	if (empty($FMEDIO)) { $FMEDIO="00" ;}
	if ($FMEDIO!="00") {
			$FILTRO_MP=" AND ID_TRN IN(SELECT ID_TRN FROM MGR_OVRD WHERE RSN=".$FMEDIO.") ";
	}
	...
	if ($FOPERA!=0) {
		$FILTRO_OPERA=" AND ID_OPR=".$FOPERA ;
					...
					$CONSULTA2="SELECT MIN(TS_TRN_END) AS MFECHA FROM TR_TRN WHERE ID_TRN IN (SELECT ID_TRN FROM MGR_OVRD WHERE RSN IS NOT NULL ) AND FL_VD<>1 AND FL_CNCL<>1";
					...
					$CONSULTA2="SELECT MAX(TS_TRN_END) AS MFECHA FROM TR_TRN WHERE ID_TRN IN (SELECT ID_TRN FROM MGR_OVRD WHERE RSN IS NOT NULL ) AND FL_VD<>1 AND FL_CNCL<>1";
					$RS2 = sqlsrv_query($conn, $CONSULTA2);
					//oci_execute($RS2);
					if ($row = @sqlsrv_fetch_array($RS2)){
...
<td >
        ...
        <table width="100%" id="Filtro">
          <tr>
            <td>
                <form action="reg_override.php" method="post" name="frmbuscar" id="frmbuscar">
                                             ...
                                             <select name="MES_ED" id="MES_ED">
                                              ...
                                              <label for="FECHA_EM_H" >Hasta</label>
                                               ...
                                               <input name="LIMPIAR" type="button" id="LIMPIAR" value="Limpiar" onClick="pagina('reg_override.php');">

                        <select style="clear:left;" name="FMEDIO" onChange="document.forms.frmbuscar.submit();">
                                                    <option value="00">Autorizaci&oacute;n</option>
                                                    <?php 
                                                     $SQLFILTRO="SELECT RSN FROM MGR_OVRD WHERE  ID_TRN IN(SELECT ID_TRN FROM TR_TRN  ".$F_FECHA." ) AND RSN IS NOT NULL GROUP BY RSN";
                                                    $RSF = sqlsrv_query($conn, $SQLFILTRO);
                                                    //oci_execute($RSF);
                                                    while ($rowF = sqlsrv_fetch_array($RSF)) {
                                                        $FLTID_RSN = $rowF['RSN'];
                                                        $S2="SELECT * FROM MGR_OVRD_RSN WHERE ID_RSN='".$FLTID_RSN."'";
                                                        $RS2 = sqlsrv_query($conn, $S2);
                                                        //oci_execute($RS2);
                                                        if ($row2 = sqlsrv_fetch_array($RS2)) {
                                                            $FLTDES_RSN_ES = $row2['DES_RSN_ES'];
                                                        }	
                                                     ?>
                                                    <option value="<?php echo $FLTID_RSN ?>" <?php  if ($FLTID_RSN==$FMEDIO) { echo "SELECTED";}?>><?php echo $FLTDES_RSN_ES ?></option>
                                                    <?php 
                                                    }
                                                     ?>
                                                    </select>
		...
		//oci_execute($RS);
				...
				$CONSULTA="SELECT COUNT(*) AS CUENTA FROM TR_TRN ".$F_FECHA." ".$FILTRO_FLAGS." AND ID_TRN IN(SELECT  ID_TRN FROM MGR_OVRD WHERE RSN IS NOT NULL ) ".$FILTRO_TIENDA.$FILTRO_TERM.$FILTRO_OPERA.$FILTRO_MP;
				...
				//CORRECCION POR REASON NULL EN EAMTRAN
				//$CONSULTA="SELECT * FROM (SELECT  a.*, ROWNUM rnum FROM (SELECT * FROM TR_TRN ".$F_FECHA." AND  ID_TRN IN(SELECT ID_TRN FROM MGR_OVRD) ".$FILTRO_FLAGS.$FILTRO_TIENDA.$FILTRO_TERM.$FILTRO_OPERA.$FILTRO_MP." ORDER BY ID_TRN  DESC) a WHERE ROWNUM <= ".$LSUP.") WHERE rnum >=  ".$LINF;
				
				//oci_execute($RS);
				$CONSULTA= "SELECT * FROM (SELECT *,ROW_NUMBER() OVER (PARTITION BY ".$CTP." ORDER BY ID_TRN DESC) ROWNUMBER FROM TR_TRN ".$F_FECHA." AND  ID_TRN IN(SELECT ID_TRN FROM MGR_OVRD) ".$FILTRO_FLAGS.$FILTRO_TIENDA.$FILTRO_TERM.$FILTRO_OPERA.$FILTRO_MP.") AS TABLEWITHROWNUMBER WHERE ROWNUMBER BETWEEN ".$LINF." AND ".$LSUP." ";

				$RS = sqlsrv_query($conn, $CONSULTA);
               ?>
               
               <p class="ArtsBubble">N&deg; TRX: <?php echo $TOTALREG;?></p>
				...
				while ($row = sqlsrv_fetch_array($RS)){
                        ...
                        $AI_TRN = $row['AI_TRN'];
								...
								$S2="SELECT * FROM MGR_OVRD WHERE ID_TRN=".$ID_TRN;
								$RS2 = sqlsrv_query($conn, $S2);
								//oci_execute($RS2);
								if ($row2 = sqlsrv_fetch_array($RS2)) {
									$RSN = $row2['RSN']; 
								...
								$S2="SELECT * FROM MGR_OVRD_RSN WHERE ID_RSN=".$RSN;
								$RS2 = sqlsrv_query($conn, $S2);
								//oci_execute($RS2);
								if ($row2 = sqlsrv_fetch_array($RS2)) {
									$AUTORIZA = $row2['DES_RSN_ES']; 
								} else {
									$AUTORIZA = "NO DEFINIDO";
								}
               ...
               ?>
                ...
                <tr>
                    ...
                    if ($LINF>=$CTP+1) {
						...
						$FILA_ANT=$LSUP-$CTP;
                   ?>
                    <input name="ANTERIOR" type="button" value="Anterior"  onClick="pagina('reg_override.php?LSUP=<?php echo $FILA_ANT?>&LINF=<?php echo $ATRAS?>&FTIENDA=<?php echo $FTIENDA?>&FTERM=<?php echo $FTERM?>&FOPERA=<?php echo $FOPERA?>&DIA_ED=<?php echo $DIA_ED ?>&MES_ED=<?php echo $MES_ED ?>&ANO_ED=<?php echo $ANO_ED ?>&DIA_EH=<?php echo $DIA_EH ?>&MES_EH=<?php echo $MES_EH ?>&ANO_EH=<?php echo $ANO_EH ?>');">
                    <?php
                    }
                    if ($LSUP<=$TOTALREG) {
						$ADELANTE=$LSUP+1;
