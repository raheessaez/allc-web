...
<?php
	...
	if (!empty($B_SERIE)) {
		$FILTRO_NSERIE=" AND SRL_NBR Like '%".strtoupper($B_SERIE)."%' " ;
					...
					$CONSULTA2="SELECT MIN(TS_TRN_END) AS MFECHA FROM TR_TRN WHERE FL_VD<>1 AND FL_CNCL<>1 AND ID_TRN IN (SELECT ID_TRN FROM TR_LTM_MOTO_DT)";
...
<?php }?>
		...
		?>
              ...
              <!-- FIN FILTRO, INICIO LISTADO -->
                ...
                <td>
					...
					$CONSULTA= "SELECT * FROM (SELECT *,ROW_NUMBER() OVER (PARTITION BY ".$CTP." ORDER BY ID_TRN DESC) ROWNUMBER FROM TR_LTM_MOTO_DT ".$F_FECHA.$FILTRO_TIENDACVM.$FILTRO_NSERIE.$FILTRO_FACT.$FLT_GERENTE.") AS TABLEWITHROWNUMBER WHERE ROWNUMBER BETWEEN ".$LINF." AND ".$LSUP."";
				   ...
				   ?>
                    ...
                    while ($row = sqlsrv_fetch_array($RS)){
						...
						if ($row2 = sqlsrv_fetch_array($RS2)) {
							...
							$RES_TICKET = date_format($FECHA_TICKET,"d/m/Y");
        ...
        <h2>Carta de Venta Veh&iacute;culo Serie <?= $SRL_NBR_CV;?></h2>
                ...
                <tr><td>
                                ...
                                <form name="frmactcliente" method="post" action="reg_cvmoto_reg.php">
                                <tr id="DIRECC">
                                ...
                                <tr id="TELEFONO">
                                    ...
                                    <td ><input name="TEL_CPR" type="text" size="20" maxlength="20" value="<?= $TEL_CPR?>"> </td>
                                </tr>
                                <tr id="CORREO">
                               ...
                               <tr>
                                    ...
                                    <input style="display:block; width:100%; margin-top:10px" type="button" name="GENERA" value="Generar Carta de Venta" onClick="VentanaImprime('PrintCVMT.php?SN=<?=$SRL_NBR_CV?>&TR=<?=$ID_TRN?>&CL=<?=$ID_CPR?>&FC=<?php echo $FL_CP?>&AI=<?php echo $AI_LN_ITM_CV?>&FAC=<?php echo $INVC_NMB_CV?>&IU=<?php echo $SESIDUSU?>');">
                                    </td>
                                </tr>
                                </form>
                        </table>
