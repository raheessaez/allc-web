...
<?php
	...
	if (!empty($B_CLTE)) {
		$FILTRO_CLTE=" AND ID_DEVS IN(SELECT ID_DEVS FROM DV_DEVCLTE WHERE COD_CLIENTE IN(SELECT COD_CLIENTE FROM DV_CLIENTE WHERE (UPPER(LTRIM(IDENTIFICACION)) Like '%".strtoupper($B_CLTE)."%') OR (UPPER(LTRIM(NOMBRE)) Like '%".strtoupper($B_CLTE)."%')  OR (UPPER(LTRIM(APELLIDO_P)) Like '%".strtoupper($B_CLTE)."%')  OR (UPPER(LTRIM(APELLIDO_M)) Like '%".strtoupper($B_CLTE)."%')  ))" ;
	}
	*/
			...
			if($D_GFC==3){
				...
				if ($BOPCION==2) {
					...
					if ($row = sqlsrv_fetch_array($RS2)){
							...
							@$date = date_create($MIN_FECHA_NCEMS);
							$MIN_FECHA_NCEMS = @date_format($date, 'd/m/Y');
					}
					...
					if ($row = sqlsrv_fetch_array($RS2)){
							...
							@$date = date_create($MAX_FECHA_NCEMS);
							$MAX_FECHA_NCEMS = @date_format($date, 'd/m/Y');
					}
		...
		<?php if($LIST==1){ ?>
              ...
              <!-- FIN FILTRO, INICIO LISTADO -->
                ...
                <td>
					...
					$RS = sqlsrv_query($conn,$SQL);
					//CUENTA REGISTROS
					$CONSULTA="SELECT COUNT(ID_DEVS) AS CUENTA FROM DV_TICKET  ".$FNC_FECHA.$FILTRO_TIENDANC.$FILTRO_TIPONC.$FILTRO_NC;
                                                    ...
                                                    <?php } ?>
														...
														while ($rowi = sqlsrv_fetch_array($RSi)) {
															...
															@$TY_REGITM = $rowi['TY_REGITM'];
																		...
																		$RSi = sqlsrv_query($arts_conn,$SQL1); 
