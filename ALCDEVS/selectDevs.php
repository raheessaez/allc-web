...
<script language="javascript">
		...
		<?php 
			if ($NUMERO==100) {
						...
						<?php
						$CONSQL="SELECT COD_TIENDA, DES_TIENDA, DES_CLAVE FROM MN_TIENDA WHERE COD_TIENDA IN(SELECT COD_TIENDA FROM MN_NEGTND WHERE COD_NEGOCIO=".$VALOR.") AND COD_TIENDA IN(SELECT COD_TIENDA FROM US_USUTND WHERE IDUSU=".$IDUSU." AND COD_NEGOCIO=".$VALOR.") ORDER BY DES_CLAVE ASC";
