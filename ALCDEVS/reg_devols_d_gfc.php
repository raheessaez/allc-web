					<?php if(empty($TCKTSEL) && empty($C_DEV)) { ?>
                    <table width="100%" id="Filtro">
                    <form action="reg_devols.php?D_GFC=<?=$D_GFC ?>&BUSCAR=1" method="post" name="forming" id="forming">
                    <tr>
                    <td>
										<?php
                                            if($CTATND==1){
                                        ...
                                        <?php if(!empty($COD_TIENDA_SEL)){?>
                                                <select name="FTERM" onChange="document.forms.forming.submit();">
                                                    ...
                                                    while ($rowF = sqlsrv_fetch_array($RSF)) {
                                                        ...
                                                        ?>
                                                        <option value="<?=$FLT_ID_WS ?>" <?php  if ($FLT_ID_WS==$FTERM) { echo "SELECTED";}?>><?=$FLTDES_WS ?></option>
                                                        <?php 
                                                    }
                                                    ?>
                                                </select>
                                                <select name="FOPERA" onChange="document.forms.forming.submit();">
                                                    ...
                                                    while ($rowF = sqlsrv_fetch_array($RSF)) {
                                                        ...
                                                        ?>
                                                        <option value="<?=$FLT_ID_OPR ?>" <?php  if ($FLT_ID_OPR==$FOPERA) { echo "SELECTED";}?>><?=$FLTCD_OPR ?></option>
                                                        <?php 
                                                    }
                                                    ?>
                                                </select>
                                   ...
                                   <select name="MES_ED" id="MES_ED">
                                    ...
                                    <input name="DIA_EH" type="text" id="DIA_EH" value="<?=$DIA_EH ?>" size="2" maxlength="2" onKeyPress="return acceptNum(event);">
                                		...
                                		<label style="clear:left" for="B_TICKET"><?=$TICKET_FACT?> </label>
                                  		<input name="B_TICKET" type="text"  id="B_TICKET" value="<?=$B_TICKET ?>" size="9" maxlength="9" onKeyPress="return acceptNum(event);">
                                        <?php if($D_GFC!=3){?>
                                        ...
                                        <?php }?>
                                        <input name="BUSCAR" type="submit" id="BUSCAR" value="Buscar <?=$TICKET_FACT?>">
                                        <input name="LIMPIAR" type="button" id="LIMPIAR" value="Limpiar" onClick="pagina('reg_devols.php?D_GFC=<?=$D_GFC ?>');">

                    </td>
                    </tr>
                    </form>
                    </table>
                    <?php } //FIN TCKSEL empty ?>
...
<table style="margin:10px 20px; ">
    ...
    <td>
                    ...
                    $CONSULTA="SELECT COUNT(ID_TRN) AS CUENTA FROM TR_TRN ".$F_FECHA." ".$FILTRO_FLAGS." AND ID_TRN IN(SELECT ID_TRN FROM TR_RTL WHERE QU_UN_RTL_TRN>0 ".$FILTRO_FACT.$FILTRO_DGFC.") ".@$FILTRO_TIENDA.$FILTRO_TERM.$FILTRO_OPERA.$FILTRO_TICKET;
                   ...
                   ?>
                    ...
                    <tr>
                        ...
                        <?php } else { ?>
                                ...
                                <?php } ?>
                        <?php }?>
                                <td id="TcktM<?=$ID_TRN ?>" class="tdShow" style="text-align:right; font-size:14pt" onClick="Mostrar<?=$ID_TRN?>();"><?=$ELTICKET." / ".$ID_TRN?></td>
                                <td id="TcktO<?=$ID_TRN ?>" class="tdHide" style="display:none; text-align:right; font-size:14pt" onClick="Ocultar<?=$ID_TRN?>();"><?=$ELTICKET?></td>
                        <td id="TRN1<?=$ID_TRN ?>"><?=$COD_TIENDA_F?></td>
                        <td id="TRN2<?=$ID_TRN ?>"><?=$TERMINAL_F?></td>
                        <td id="TRN3<?=$ID_TRN ?>"><?=$OPERADOR_F?></td>
                        <td id="TRN4<?=$ID_TRN ?>"><?=$TIPO_MEDPAGO?></td>
                        <td id="TRN5<?=$ID_TRN ?>"  style="text-align:right"><?=$MONEDA.$IMPORTE?></td>
                        <td id="TRN6<?=$ID_TRN ?>"  style="text-align:right"><?=$TS_TICKET?></td>
                        <td id="TRN7<?=$ID_TRN ?>"  style="border-left-width:3px; border-left-style:solid; border-left-color:#DFDFDF;text-align:right"><?=$TS_TRN_BGN?></td>
                        <td id="TRN8<?=$ID_TRN ?>"  style="text-align:right"><?=$TS_TRN_END?></td>
                        ...
                        <?php } ?>
							...
							<?php if(!empty($B_TICKET)){?>
                                <?php if($NO_DEVS==0){?>
                                    <tr id="ver<?=$ID_TRN?>" style="display:table-row">
                                <?php } else {?>
                            ...
                            <?php }?>
                                <td colspan="10" style="background-color:#FFF">
                                           ...
                                           <?php include("reg_devols_sel_art.php");?>
                                            <!-- DETALLE DE ITEMS EN TRX -->
                    ...
                    <tr>
                        ...
                        if ($LINF>=$CTP+1) {
                            ...
                            $FILA_ANT=$LSUP-$CTP;
                       ?>
                        <input name="ANTERIOR" type="button" value="Anterior"  onClick="pagina('reg_devols.php?D_GFC=<?=$D_GFC ?>&BUSCAR=<?=$BUSCAR?>&LSUP=<?=$FILA_ANT?>&LINF=<?=$ATRAS?>&COD_NEGOCIO=<?=$COD_NEGOCIO_SEL?>&COD_TIENDA=<?=$COD_TIENDA_SEL?>&FTERM=<?=$FTERM?>&FOPERA=<?=$FOPERA?>&DIA_ED=<?=$DIA_ED ?>&MES_ED=<?=$MES_ED ?>&ANO_ED=<?=$ANO_ED ?>&DIA_EH=<?=$DIA_EH ?>&MES_EH=<?=$MES_EH ?>&ANO_EH=<?=$ANO_EH ?>&B_TICKET=<?=$B_TICKET ?>');">
                        <?php
                        }
                        if ($LSUP<=$TOTALREG) {
                            $ADELANTE=$LSUP+1;
                            $FILA_POS=$LSUP+$CTP;
                       ?>
                        <input name="SIGUIENTE" type="button" value="Siguiente" onClick="pagina('reg_devols.php?D_GFC=<?=$D_GFC ?>&BUSCAR=<?=$BUSCAR?>&LSUP=<?=$FILA_POS?>&LINF=<?=$ADELANTE?>&COD_NEGOCIO=<?=$COD_NEGOCIO_SEL?>&COD_TIENDA=<?=$COD_TIENDA_SEL?>&FTERM=<?=$FTERM?>&FOPERA=<?=$FOPERA?>&DIA_ED=<?=$DIA_ED ?>&MES_ED=<?=$MES_ED ?>&ANO_ED=<?=$ANO_ED ?>&DIA_EH=<?=$DIA_EH ?>&MES_EH=<?=$MES_EH ?>&ANO_EH=<?=$ANO_EH ?>&B_TICKET=<?=$B_TICKET ?>');">
                        <?php }?>
                   ...
                   <?php include("reg_devols_t_sel.php");?>
                </td>
                </tr>
                </table>